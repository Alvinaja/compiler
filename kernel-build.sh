...
LLD_VER="$("$GCC64_DIR"/bin/aarch64-elf-ld.lld -v | head -n1 | sed 's/(compatible with [^)]*)//' | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//')"
...
make -j$(nproc) ARCH=arm64 O=out \
  ...
  git clone --depth=1 https://github.com/Himemoria/AnyKernel3 -b merlin AnyKernel
    cp $IMAGE AnyKernel
    cp $DTBO AnyKernel
